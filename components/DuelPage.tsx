...
const DuelPage: React.FC<DuelPageProps> = ({ settings, onEndDuel }) => {
    ...
    // Handles submission of a new challenge
    const handleSubmitChallenge = useCallback(async (msg1: DuelMessage, msg2: DuelMessage, argument: string, wager: number) => {
        setIsChallengeModalOpen(false);
        setIsLoading(true);
        
        // 1. Deduct cost from player
        setPlayerScore(prev => prev - wager);
        
        // 2. Call AI to analyze the challenge
        const result = await analyzeChallenge({ msg1, msg2, argument, wager }, history, settings);
        
        // 3. Create the full challenge result object for history
        ...
        // 4. Update scores based on result
        let systemMessageText = '';
        let systemMessageDetails = '';

        if (result.isApproved) {
            const finalPenalty = Math.min(aiScore, result.penalty); // AI can't go below 0
            setPlayerScore(prev => prev + wager); // Refund player
        ...
        // 5. Add a system message to the chat history
        const systemMessage: SystemMessage = {
            id: `sys-${Date.now()}`,
            type: 'SYSTEM',
            text: systemMessageText,
            details: systemMessageDetails,
            isSuccess: result.isApproved,
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Ensure first turn score is 10, even if AI fails to follow instructions
    ...
    const challengeCosts = [1, 3, 10]; // Max cost for 3rd challenge is handled in modal
